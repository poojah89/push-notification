package com.aut.pushnotification.config;
...
public class JWTFilterConfig extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (path.contains("swagger") || path.contains("docs") || path.contains("timeinYYYYformat")
				|| path.contains("timeinYYYformat")) {
		...
		if (!stringContainsItemFromList(path, NO_TOKEN)) {
			...
			try {
				...
				logger.debug("module  :" + authToken.getModule());
				logger.info("traderId  :" + authToken.getTraderId());
